...
def get_table_request(table_name):
    ...
    while True:
        if token in responseData:
            ...
            return jsonify({"table_data": response}),
        time.sleep(0.1)
...
def create_NFForward_request():
    ...
    while True:
        if token in responseData:
            response = responseData.pop(token)
            return jsonify({"response": response}), (200 if response == 'OK' else 500)
        time.sleep(0.1)
...
@app.route("/api/setUpstreamMode4", methods=["POST"])
def create_setUpstreamMode4_request():
    token = str(uuid.uuid4())
    requestData = request.json
    print("got request", requestData)
    data = ["setUpstreamMode4", requestData]
    requestQueue.put((data, token))
    while True:
        if token in responseData:
            response = responseData.pop(token)
...
@app.route("/api/setDownstreamMode4", methods=["POST"])
def create_setDownstreamMode4_request():
    token = str(uuid.uuid4())
    requestData = request.json
    print("got request", requestData)
    data = ["setDownstreamMode4", requestData]
    requestQueue.put((data, token))
    while True:
        if token in responseData:
            response = responseData.pop(token)
...
@app.route("/api/setD6GService", methods=["POST"])
def create_setD6GService_request():
    token = str(uuid.uuid4())
    requestData = request.json
    print("got request", requestData)
    data = ["setD6GService", requestData]
    requestQueue.put((data, token))
    while True:
        if token in responseData:
            response = responseData.pop(token)
...
@app.route("/api/UEMapping", methods=["POST"])
def create_setUEMapping_request():
    token = str(uuid.uuid4())
    requestData = request.json
    print("got request", requestData)
    data = ["UEMapping", requestData]
    requestQueue.put((data, token))
    while True:
        if token in responseData:
            response = responseData.pop(token)
...
@app.route("/api/arp_reply", methods=["POST"])
def create_arp_reply_request():
    token = str(uuid.uuid4())
    requestData = request.json
    print("got request", requestData)
    data = ["arp_reply", requestData]
    requestQueue.put((data, token))
    while True:
        if token in responseData:
            response = responseData.pop(token)
...
@app.route("/api/icmp_reply", methods=["POST"])
def create_icmp_reply_request():
    token = str(uuid.uuid4())
    requestData = request.json
    print("got request", requestData)
    data = ["icmp_reply", requestData]
    requestQueue.put((data, token))
    while True:
        if token in responseData:
            response = responseData.pop(token)
