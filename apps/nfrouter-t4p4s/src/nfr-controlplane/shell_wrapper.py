...
def NFForward(sh, initialParameters):
    ...
    parameters = {
        ...
        "value": [str(socket.htons(initialParameters["serviceId"])), str(socket.htons(initialParameters["locationId"])), str(socket.htons(initialParameters["nextNF"]))],
        "paramName": ["port", "dstMAC"],
        "actionParam": [str(socket.htons(initialParameters["port"])), str(initialParameters["dstMAC"])],
    }
    result, entry = insertTableEntry(sh, parameters)
    return result
...
def setupConnection(
    ...
    deviceID=1,
    electionID=(0, 1),
    p4infoFile="config/nfrouting.p4runtime.txt",
    binFile="../data-plane/p4-v1model/nfrouting.json",
):
    sh.setup(
        device_id=deviceID,
